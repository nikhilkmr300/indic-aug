...
def gen_neighborhood(args):
    ...
    lines = [tuple(s.strip().split('\t'))
             for s in open(args.in_file).readlines()]
    num_lines = len(lines)
    ...
    if args.num_shards != 1:
        ...
        if output_labels:
            l_rec_file = open(args.output_prefix + '_' +
                              str(args.shard) + '.label', 'w')
    else:
    ...
    while (sents != []):
        ...
        toks = [F.pad(tok, (0, max_len - len(tok)), 'constant', pad_tok)
                for tok in toks]
        masks = [F.pad(mask, (0, max_len - len(mask)),
                       'constant', pad_tok) for mask in masks]
        ...
        rec, rec_masks = hf_reconstruction_prob_tok(
            toks, masks, tokenizer, r_model, softmax_mask, reconstruct=True, topk=args.topk)

        # decode reconstructions and append to lists
        for i in range(len(rec)):
            rec_work = rec[i].cpu().tolist()
            s_rec = [s.strip() for s in tokenizer.decode(
                [val for val in rec_work if val != tokenizer.pad_token_id][1:-1]).split(tokenizer.sep_token)]
            s_rec = tuple(s_rec)
