...
def hf_masked_encode(
    ...
    tokens = np.asarray(tokenizer.encode(
        sentence, *addl_sentences, add_special_tokens=True))
    ...
    if (reconstruct):
        ...
        if topk != -1:
            ...
            if (len(masked_index) > 1):
                samples = torch.cat([idx[torch.multinomial(kprob, 1)]
        ...
        # unrestricted sampling
            ...
            if (len(masked_index) > 1):
                samples = torch.cat([torch.multinomial(prob, 1)
                                    for prob in probs])
            else:
