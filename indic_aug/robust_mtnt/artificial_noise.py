...
FR_EN_STOPS = "./en_hi_stopwords.txt"
...
def add_err_spl(fr_word, en_word):
    ...
    if np.random.random() < 0.5:
        ...
        fr_word = "".join(chars)
    print("word after spell hi:{}, en:{}".format(fr_word, en_word))
...
def add_err_emo(fr_word, en_word):
    emoji_list = list(emojis.db.get_emoji_aliases().keys())
    # inserting emoticon repeated thrice
    emoji_choice_key = np.random.choice(emoji_list)
    # removing the front and last ':'
    emoji_choice_key = emoji_choice_key[1:-1]
    emoji = emojis.db.get_emoji_by_alias(emoji_choice_key).emoji
    emoji_choice = "".join(emoji * 3)
    ...
    with codecs.open(input_file_src, 'r', encoding='utf-16') as f_src_in, \
        ...
        for fr_line, en_line in zip(f_src_in.readlines(), f_tgt_in.readlines()):
            ...
            for fr_word, en_word in zip(fr_line.split(), en_line.split()):
                noise_id = np.random.choice(
                    np.arange(0, len(noise_proba)), p=noise_proba)
...
if __name__ == "__main__":
    ...
    add_noise(input_file_src, input_file_tgt, output_file_src,
              output_file_tgt, noise_proba, noise_funcs)
